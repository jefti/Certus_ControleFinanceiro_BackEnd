...
| Domínio | Recursos |
| --- | --- |
| 🔐 **Autenticação** | Login com JWT, recuperação de senha por e-mail (código de 6 dígitos), expiração configurável |
...
| 💳 **Faturamentos** | Parcelas/competências geradas a partir do título, com status (em aberto, pago, atrasado) e validação de pagamento |
...
├── ⚙️  service        → Regras de negócio (inclui RecorrenciaCalculator)
├── 💾 repository     → Acesso a dados (Spring Data JPA)
├── 🧱 entity         → Modelo de domínio (JPA / Hibernate) + enums
├── 🔁 dto            → Requests, responses e mappers (ModelMapper)
